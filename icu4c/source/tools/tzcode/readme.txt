...
3. Build ICU normally. You will see a notice "updating zoneinfo.txt..."
   ...
   The tarball is unpacked into, and all files generated by these tools are
   written to, the "out" subdirectory of this directory, which is removed by
   "make clean".  The generated time zone data therefore ends up in

     {path_to}/icu/source/tools/tzcode/out/zoneinfo64.txt

   and the ICU data build picks it up from there instead of the checked-in
   {path_to}/icu/source/data/misc/zoneinfo64.txt.
...
5. Run make target "check-dump".  This target extract makes the original
   ...
   transitions for all ICU timezone to files under out/zdumpout /
   out/icuzdumpout directory.  When they produce different results, the
   target returns the error.

6. Don't forget to copy the new zoneinfo64.txt (from its location at
   {path_to}/icu/source/tools/tzcode/out/zoneinfo64.txt) over
   {path_to}/icu/source/data/misc/zoneinfo64.txt and check it in.
