...
// Mapping from a TZ database zone ID to its CLDR region. Zones which are
// known to CLDR, but which are not associated with any region, are mapped
// to an empty string.
typedef map<string, string> CldrZoneRegionMap;

/**
 * Read the zone/region mapping from the CLDR BCP 47 time zone identifier
 * file (common/bcp47/timezone.xml).
 *
 * As described in https://unicode.org/reports/tr35/#Time_Zone_Identifiers,
 * the first two letters of a length 5 short identifier double as the time
 * zone's associated region, unless the time zone has an explicit "region"
 * attribute that overrides this. Short identifiers of length not equal to 5
 * are not associated with a region, unless the time zone has an explicit
 * "region" attribute. The "alias" attribute lists all TZ database IDs
 * (including the CLDR canonical one) mapping to the short identifier.
 */
void readCldrTimeZones(const string& filepath, CldrZoneRegionMap& zoneRegions) {
    ...
    while (getline(in, line)) {
        if (!inTzKey) {
            if (line.find("<key name=\"tz\"") == string::npos) {
                continue;
            }
            inTzKey = true;
        ...
        size_t pos = 0;
        while ((pos = line.find("<type ", pos)) != string::npos) {
            size_t end = line.find('>', pos);
            string tag = (end == string::npos)
                ? line.substr(pos) : line.substr(pos, end - pos + 1);
            pos = (end == string::npos) ? line.size() : end + 1;

            map<string, string> attrs = parseXmlAttributes(tag);
            map<string, string>::const_iterator alias = attrs.find("alias");
            if (alias == attrs.end()) {
                // Deprecated short identifiers have no TZ database ID.
                continue;
            }
            const string& name = attrs["name"];
            string region;
            map<string, string>::const_iterator regionAttr = attrs.find("region");
            if (regionAttr != attrs.end()) {
                region = regionAttr->second;
            } else if (name.length() == 5) {
                region = name.substr(0, 2);
                for (size_t i = 0; i < region.length(); ++i) {
                    region[i] = static_cast<char>(toupper(region[i]));
                }
            }

            istringstream ids(alias->second);
            string id;
            while (ids >> id) {
                zoneRegions[id] = region;
            }
        }
    }
}
...
int main(int argc, char *argv[]) {
    string rootpath, timezonesPath, version;
    ...
    if (!validArgs) {
        cout << "Usage: tz2icu <dir> <timezones> <tzver> <metazones> [--old]" << endl
             << " <dir>        path to zoneinfo file tree generated by" << endl
             << "              ICU-patched version of zic" << endl
             << " <timezones>  path to CLDR common/bcp47/timezone.xml," << endl
             << "              providing the zone/region mapping" << endl
    ...
    CldrZoneRegionMap zoneRegions;
    try {
        readCldrTimeZones(timezonesPath, zoneRegions);
        cout << "Finished reading " << zoneRegions.size()
             << " zone/region mappings from " << timezonesPath << endl;
    } catch (const exception& error) {
        cerr << "Error: While reading " << timezonesPath << ": " << error.what() << endl;
        return 1;
    }
    if (zoneRegions.empty()) {
        cerr << "Error: No zone/region mappings found in " << timezonesPath << endl;
        return 1;
    }
    ...
    int32_t n = 0;
    for (CldrZoneRegionMap::const_iterator i = zoneRegions.begin();
         i != zoneRegions.end(); ++i) {
        ...
        if (country.empty()) {
            // The zone is explicitly not associated with any region,
            // for example "utcw01" (Etc/GMT+1).
            continue;
        }
    ...
    cout << "Finished mapping " << n
         << " zones to regions from " << timezonesPath << endl;
